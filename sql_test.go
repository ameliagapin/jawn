...
type sqlJawnCoder interface {
	sql.Scanner
	Value() (driver.Value, error)
}

func doSQLTest(t *testing.T, val1, val2 interface{}, jawnVal1, jawnVal2 sqlJawnCoder) {
	val, err := jawnVal1.Value()
	assert.NoError(t, err)
	assert.EqualValues(t, val1, val) // sql.Null* types return int64 for all integer types

	assert.NoError(t, jawnVal2.Scan(val2))
	val, err = jawnVal2.Value()
	assert.NoError(t, err)
...
func TestSQLNullDuration(t *testing.T) {
	v1, v2 := time.Duration(7676), time.Duration(7676)
	doSQLTest(t, v1, v2, &NullDuration{Duration: v1, Valid: true}, &NullDuration{})

	doSQLTest(t, nil, nil, &NullDuration{Valid: false}, &NullDuration{})
	doSQLTest(t, nil, nil, &NullDuration{Valid: false}, &NullDuration{Valid: true})
}
